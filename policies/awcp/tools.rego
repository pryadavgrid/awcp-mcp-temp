...
#
# The OPA Agent (awcp-agents/opa_agent) sends an `input` describing ONE tool call
...
# per tool — the tier comes from the operator's control-plane setting.
#
#   input.tool         "web_search"               the tool being called
#   input.risk_tier    "low|medium|high|severe"   its operator-set tier
