...
matching the agent's own runtime-script path among live processes, and the
...
def _info(port: int) -> dict:
    """The agent's own /info (model, framework, …). Best-effort: only a running
    agent answers, and we never assume what it reports — the model comes straight
    from /info, so nothing about it is hardcoded here."""
    try:
        with urllib.request.urlopen(f"http://localhost:{port}/info", timeout=1.0) as r:
            if r.status == 200:
        ...
        items.append(
            {
                ...
                "pids": pids,
                "port": port,
                # Surfaced live from the agent's /info (None when stopped / not reported).
...
function card(a){
  ...
  return `<div class="card" data-id="${a.id}">
    ...
    <div>
      ...
      <div class="meta">${a.running ? ('running · pid '+a.pids.join(', ')+model) : 'stopped'}</div>
    </div>
